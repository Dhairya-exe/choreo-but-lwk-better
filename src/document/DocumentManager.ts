...
export async function setupEventListeners() {
  ...
  let autoSaveNeedsCodegen = false;
  const performAutoSave = (generateCode: boolean) => {
    if (uiState.hasSaveLocation) {
      uiState.setProjectSavingState(SavingState.SAVING);
      saveProject(generateCode);
    }
  };
  const autoSaveUnlisten = reaction(
    () => doc.serializeChor(),
    (project, previousProject) => {
      const { ui: _projectUI, ...projectInputs } = project;
      const { ui: _previousProjectUI, ...previousProjectInputs } =
        previousProject;
      autoSaveNeedsCodegen ||=
        project.name === previousProject.name &&
        JSON.stringify(projectInputs) !== JSON.stringify(previousProjectInputs);
      clearTimeout(autoSaveDebounce);
      autoSaveDebounce = setTimeout(() => {
        const generateCode = autoSaveNeedsCodegen;
        autoSaveNeedsCodegen = false;
        performAutoSave(generateCode);
      }, 50);
    }
  );
...
export async function saveProject(generateCode: boolean = true) {
  if (await canSave()) {
    try {
      const tasks = [Commands.writeProject(doc.serializeChor())];
      if (generateCode) {
        tasks.push(genJavaFiles());
      }
