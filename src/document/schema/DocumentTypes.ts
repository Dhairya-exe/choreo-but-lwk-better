...
export interface CommandVisualMetadata {
  icon: string;
  color: string;
  duration: number;
  commands: CommandVisualMetadata[];
}

export interface CommandAppearanceMetadata {
  icon: string;
  color: string;
}

export interface PathSweepMetadata {
  id: string;
  name: string;
  fromWaypoint: number;
  toWaypoint: number;
...
export interface PathVisualMetadata {
  commandVisuals?: CommandVisualMetadata[];
  sweeps: PathSweepMetadata[];
  activeSweepId?: string | null;
  selectedSweepIds?: string[];
}

export interface ProjectUI {
  paths: Record<string, PathVisualMetadata>;
  commands?: Record<string, CommandAppearanceMetadata>;
}
