...
export type CommandType = CommandGroupType | "wait" | "stay" | "named" | "none";
export const CommandTypeNames = {
  ...
  stay: { id: "stay", name: "Stay in place" },
...
export const CommandStore = types
  ...
  .views((self) => ({
    ...
    get isWait(): boolean {
      return self.type === "wait" || self.type === "stay";
    },
    get isStay(): boolean {
      return self.type === "stay";
    },
    get stayDurationSeconds(): number {
      if (self.type === "stay") {
        return Math.max(0, Math.abs(self.time.value));
      }
      if (commandTypeIsGroup(self.type)) {
        return self.commands.reduce(
          (duration, command) =>
            Math.max(duration, command.stayDurationSeconds),
          0
        );
      }
      return 0;
