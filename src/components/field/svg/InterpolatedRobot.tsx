...
import { playbackToTrajectoryTime } from "../PlaybackTiming";
...
class InterpolatedRobot extends Component<Props, State> {
  ...
  render() {
    ...
    const trajectoryTime = playbackToTrajectoryTime(
      doc.pathlist.activePath,
      this.props.timestamp
    );
    if (!trajectoryTimeIsVisible(doc.pathlist.activePath, trajectoryTime)) {
      return <></>;
    }
    const pose1 = sample(trajectoryTime, trajectory);
