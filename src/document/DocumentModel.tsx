...
import { CommandAppearanceStore } from "./CommandAppearanceStore";
import { ICommandStore } from "./CommandStore";
...
export const DocumentStore = types
  .model("DocumentStore", {
    ...
    commandAppearances: types.map(CommandAppearanceStore),
    loadedProjectUI: types.frozen<ProjectUI>({ paths: {}, commands: {} }),
  ...
  .views((self) => ({
    ...
    serializeChor(): Project {
      const pathVisuals = Object.fromEntries(
        Array.from(self.pathlist.paths.values()).map((path) => [
          path.name,
          path.serializeVisual
        ])
      );
      return {
        name: self.name,
        ...
        ui: {
          ...
          commands: Object.fromEntries(
            Array.from(self.commandAppearances.values()).map((appearance) => [
              appearance.name,
    ...
    get globalCommandNames(): string[] {
      const names = new Set(self.commandAppearances.keys());
      const visit = (command: ICommandStore) => {
        if (command.isNamed && command.name?.trim()) {
          names.add(command.name.trim());
        }
        command.commands.forEach(visit);
      };
      self.pathlist.paths.forEach((path) =>
        path.markers.forEach((marker) => visit(marker.event))
      );
      return Array.from(names).sort((a, b) => a.localeCompare(b));
    },
    getCommandAppearance(name: string | null | undefined) {
      return name ? self.commandAppearances.get(name) : undefined;
    }
  }))
  .volatile((self) => ({
  ...
  .actions((self) => ({
    deserializeChor(ser: Project) {
      ...
      self.type = ser.type;
      self.loadedProjectUI = ser.ui ?? { paths: {}, commands: {} };
      self.commandAppearances.clear();
      Object.entries(ser.ui?.commands ?? {}).forEach(([name, appearance]) => {
        self.commandAppearances.put({ name, ...appearance });
      });
    },
    applyProjectUI() {
      self.pathlist.paths.forEach((path) => {
        const pathVisual = self.loadedProjectUI.paths[path.name];
        path.applyVisualMetadata(pathVisual);
        const migrateCommandVisual = (
          command: ICommandStore,
          visual:
            | import("./schema/DocumentTypes").CommandVisualMetadata
            | undefined
        ) => {
          ...
          if (
            command.isNamed &&
            name &&
            !self.commandAppearances.has(name) &&
            visual
          ) {
            self.commandAppearances.put({
              name,
              icon: visual.icon || "bolt",
              color: visual.color || "#64d2ff"
            });
          }
          command.commands.forEach((child, index) =>
            migrateCommandVisual(child, visual?.commands?.[index])
          );
        };
        path.markers.forEach((marker, index) =>
          migrateCommandVisual(
            marker.event,
            pathVisual?.commandVisuals?.[index]
          )
        );
      });
    },
    ensureCommandAppearance(name: string) {
      const trimmedName = name.trim();
      if (!trimmedName || self.commandAppearances.has(trimmedName)) return;
      self.commandAppearances.put({
        name: trimmedName,
        icon: "bolt",
        color: "#64d2ff"
    ...
    setCommandAppearanceIcon(name: string, icon: string) {
      this.ensureCommandAppearance(name);
      self.commandAppearances.get(name.trim())?.setIcon(icon);
    },
    setCommandAppearanceColor(name: string, color: string) {
      this.ensureCommandAppearance(name);
      self.commandAppearances.get(name.trim())?.setColor(color);
    },
    deleteCommandAppearance(name: string) {
      self.commandAppearances.delete(name);
    },
    setName(name: string) {
      self.name = name;
    },
