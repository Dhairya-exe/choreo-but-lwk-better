...
import FieldCommandHighlights from "./FieldCommandHighlights";
...
import { waypointIsVisible } from "../PathSweepUtils";
import FieldSamples from "./FieldSamples";
import FieldSweepPreview from "./FieldSweepPreview";
...
class FieldOverlayRoot extends Component<Props, State> {
  ...
  private pendingTransform?: d3.ZoomTransform;

  private handleWindowResize = () => this.scheduleResize();
  private handleCenter = (e: Event) => {
    const detail = (e as CustomEvent).detail;
    this.center(detail.x, detail.y, detail.k);
  };
  ...
  componentWillUnmount(): void {
    ...
    this.fieldSelection().interrupt().on(".zoom", null).on("contextmenu", null);
  }
  ...
  private zoomed(e: any) {
    this.pendingTransform = e.transform;
    if (this.zoomFrame !== 0) return;
    this.zoomFrame = window.requestAnimationFrame(() => {
      this.zoomFrame = 0;
      const transform = this.pendingTransform;
      if (transform === undefined) return;
      this.setState(
        { xPan: transform.x, yPan: transform.y, zoom: transform.k },
        () => this.handleResize()
      );
    });
  }
  ...
  private handleResize() {
    if (this.svgRef.current === null || this.frameRef.current === null) return;
    const factor = this.getScalingFactor(this.svgRef?.current);
    if (Math.abs(uiState.fieldScalingFactor - factor) > 0.00001) {
      uiState.setFieldScalingFactor(factor);
    }
    const nextMatrix = this.frameRef.current.getScreenCTM();
    if (
      nextMatrix !== null &&
      ["a", "b", "c", "d", "e", "f"].some(
        (key) =>
          nextMatrix[key as keyof DOMMatrix] !==
          this.state.fieldMatrix[key as keyof DOMMatrix]
      )
    ) {
      this.setState({ fieldMatrix: nextMatrix });
    }
  }
        ...
        <FieldMatrixContext.Provider value={this.state.fieldMatrix}>
          ...
          >
            ...
            <g id="toFlipPath">
              ...
              <FieldSweepPreview />
              {layers[ViewLayers.Trajectory] && layers[ViewLayers.Commands] && (
                <FieldCommandHighlights />
              )}
            ...
            {layers[ViewLayers.Waypoints] &&
              doc.pathlist.activePath.params.waypoints
                .map((point, index) => {
                  ...
                  if (!waypointIsVisible(activePath, index)) {
                    return undefined;
                  }
                  if (
